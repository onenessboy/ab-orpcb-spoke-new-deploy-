...
resource "azurerm_synapse_workspace" "syn_ws" {
  ...
  sql_administrator_login_password = var.synadmin_password      //TODO - check if we can generate on the fly; we will either way use the aad admin group hence no need to pass on as param (secret)
